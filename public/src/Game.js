...
Game.Hanoi.prototype = {
    ...
    preload: function() {
        ...
        this.load.bitmapFont(
            'nokia',
            'resources/fonts/nokia16black.png',
            'resources/fonts/nokia16black.xml'
        );
    ...
    create: function() {
        ...
        this._buttonRestart = this.add.button(
            x + 220,
            y,
            'button',
            this.restartGame,
        ...
        this._buttonRestore = this.add.button(
            x + 440,
            y,
            'button',
            this.restoreGame,
        ...
        this._stAttemps = this.add.text(
            22,
            22,
            'MOVEMENTS: ' + this._attemps,
            style
        );
        ...
        for (var i = 0, x = 93; i < 3; i++, x += 320) {
            ...
            tower._pieces =
                i == 0 ? Array.from(Array(this._numberPiece).keys()).reverse() : [];
        ...
        for (
            var i = 0, scaleX = 0.9, positionY = 430, positionX = -10; i < this._numberPiece; i++, scaleX -= 0.1, positionY -= 33, positionX += 15
        ) {
            var singlePiece;

            if (i >= colors.length) {
                singlePiece = this.game.add.sprite(
                    positionX,
                    positionY,
            ...
            singlePiece.body.setSize(
                singlePiece.body.width - 200,
                singlePiece.body.height,
                91,
                1
            );
    ...
    recursionHanoi: function(n, origin, destination, auxiliary) {
        if (n == this._numberPiece - 1) {
            console.log(
                'Move disk ' + n + ' from tower ' + origin + ' to tower ' + destination
            );
...
function makeLoginRequest() {
    console.log("termina");
    let par = localStorage.getItem('user-params');
    fetch('/ingreso', {
        method: 'POST',
        redirect: 'follow',
        body: par,
